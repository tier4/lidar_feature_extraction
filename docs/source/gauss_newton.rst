...
.. math::
    E_{\rho}(\mathbf{\beta}) = \sum_{i=1}^{n} \rho(\frac{e_{i}(\mathbf{\beta})}{\sigma_{MAD}})
...
.. math::
    \sigma_{MAD}=\frac{\operatorname{MAD}}{\Phi^{-1}(\frac{3}{4})},\, \operatorname{MAD}=\operatorname{median}_{i}(\left|e_{i}−\operatorname{median}_{j}(e_{j})\right|)
